-- Initialize Pokemon species data
...
CREATE OR REPLACE FUNCTION init_user_inventory()
RETURNS TRIGGER AS $$
BEGIN
    INSERT INTO user_inventory (user_id, item_name, quantity) VALUES
    (NEW.user_id, 'pokeball', 5),
    (NEW.user_id, 'potion', 3);
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER trigger_init_inventory
    AFTER INSERT ON users
    FOR EACH ROW
    EXECUTE FUNCTION init_user_inventory();